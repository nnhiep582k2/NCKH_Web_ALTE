...
// Like home
const btnLikeProperties = document.querySelectorAll(".property_list-btn-like");
for (let index = 0; index < btnLikeProperties.length; index++) {
    btnLikeProperties[index].onclick = () => {
        if (btnLikeProperties[index].style.color == "red") {
            btnLikeProperties[index].style.color = "grey";
        } else {
            btnLikeProperties[index].style.color = "red";
        }
    }
}
